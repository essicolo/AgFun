...
# method: whether "ilr" for isometric log-ratio or "slr" for summed log-ratio 
...

{
  ...
  if (method == "ilr") {
    ...
    bal_o <- slr(comp, graph = sbp_o)
  } else {
    stop("The method argument should either be ilr or slr.")
  }
  ...
  if (show.range) {
    for (i in 1:(ncol(V))) {
      text(
        x = c(nodes[i, 1], nodes[i, 2]), y = heights[i],
  ...
  if (type == "boxplot") {
    ...
    for (i in 1:length(heights)) {
      ...
      for (j in 1:ngroup) {
        ...
        segments(
          x0 = quantile_mapped[1], y0 = heights[i] + dh_gr[i, j],
          x1 = quantile_mapped[5], y1 = heights[i] + dh_gr[i, j]
        )
        polygon(
          x = c(quantile_mapped[2], quantile_mapped[4], quantile_mapped[4], quantile_mapped[2]),
          y = c(
        ...
        segments(
          x0 = quantile_mapped[3], y0 = heights[i] + dh_gr[i, j] - dh[i] / (ngroup + 1) / 2.5,
  ...
  if (type == "conf") {
    ...
    for (i in 1:length(heights)) {
      ...
      for (j in 1:ngroup) {
        ...
        segments(
          x0 = conf_mapped[1], y0 = heights[i] + dh_gr[i, j],
          x1 = conf_mapped[3], y1 = heights[i] + dh_gr[i, j]
        )
        segments(
          x0 = conf_mapped[1], y0 = heights[i] + dh_gr[i, j] + dh[i] / (ngroup + 1) / 3,
          x1 = conf_mapped[1], y1 = heights[i] + dh_gr[i, j] - dh[i] / (ngroup + 1) / 3
        )
        segments(
          x0 = conf_mapped[3], y0 = heights[i] + dh_gr[i, j] + dh[i] / (ngroup + 1) / 3,
...
# home made slr function
slr <- function(x, graph) {
  column_indexes <- 1:ncol(x)
  balances <- matrix(data = NA, nrow = nrow(x), ncol = nrow(graph))
  for (i in 1:nrow(graph)) {
    numerator_indexes <- column_indexes[graph[i, ] == 1]
    denominator_indexes <- column_indexes[graph[i, ] == -1]
    
    # j'utilise des if parce que la somme des lignes ne focntionne
    # pas en R s'il n'y a qu'une valeur à additionner
    # il y a surement des approches plus efficaces...
    if (length(numerator_indexes) == 1) {
      numerator_sum <- x[, numerator_indexes]
    } else {
      numerator_sum <- rowSums(x[, numerator_indexes])
    }
    
    if (length(denominator_indexes) == 1) {
      denominator_sum <- x[, denominator_indexes]
    } else {
      denominator_sum <- rowSums(x[, denominator_indexes])
    }
    
    balances[, i] <- log(numerator_sum / denominator_sum)
  }
  return(balances)
}
