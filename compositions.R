close_comp = function(x, total=1) {
  t(apply(x, 1, function(x) total * x/sum(x)))
}

build_base = function(sbp) {
  W = t(sbp)
  dimW = dim(W)
  is_pos = ifelse(W > 0, 1, 0)
  is_neg = ifelse(W < 0, 1, 0)
  ones_d = matrix(rep(1, dimW[1]**2), ncol=dimW[1])
  n_pos = ones_d %*% is_pos
  n_neg = ones_d %*% is_neg
  W = is_pos * n_neg - is_neg * n_pos
  nn = matrix(ncol = ncol(W), nrow=nrow(W))
  for (i in 1:ncol(W)) {
    nn[, i] = 1/sqrt(W[, i] %*% W[, i])
  }
  V = W * nn
...
alr_ = function(comp, common_comp = NULL) {
  if (is.null(common_comp)) common_comp = ncol(comp)
  log(comp[, -common_comp] / comp[, common_comp])
}

clr_ = function(comp) {
  t(apply(comp, 1, function(x) log(x/geomean(x))))
}

ilr_ = function(comp, sbp) {
  V = build_base(sbp)
  log(comp) %*% V
}

alr_inv = function(x, common_comp = NULL, total=1) {
  if (is.null(common_comp)) common_comp = ncol(x) + 1
  rat = 1 + apply(exp(x), 1, sum)
  dat = exp(x)/rat
  comp = matrix(ncol = ncol(x)+1, nrow=nrow(x))
  comp[, -common_comp] = dat * total
  comp[, common_comp] = 1/rat * total
  return(comp)
}

clr_inv = function(x, total=1) {
  close_comp(exp(x)) * total
}

ilr_inv = function(x, sbp, total=1) {
  V = build_base(sbp)
  clr = x %*% t(V)
  close_comp(exp(clr)) * total
}
...
#####
# Give names to balances from a SBP
#####
...
compci = function(comp=NULL, bal=NULL, sbp, level=0.95, nbsim=1e5) {
  ...
  ###
  # Simulate univariate confidence intervals for compositions
  # I do not recommand doing this though
  ###
...
codadendrogram = function(comp, V, equal.height=FALSE, range=c(-4,4),
